...
public class StringUtil {
	 ...
	 * @return
	 */
	public static String subStringByBytes(String str, int maxBytesLen) {
		int reInt = 0;
		StringBuffer reStr = new StringBuffer();
		if (str == null)
			return "";
		char[] tempChar = str.toCharArray();
		for (int kk = 0; (kk < tempChar.length && maxBytesLen > reInt); kk++) {
			String s1 = str.valueOf(tempChar[kk]);
			reInt += s1.getBytes().length;
			reStr.append(s1);
		}
